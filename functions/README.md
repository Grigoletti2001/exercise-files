...
- JavaScript basics, control flow, loops, arrays, iterating, functions.
- Critical thinking
...
Write a function `sumDigits` that accepts a number and returns the sum of its digits.  If you happen to end up in a situation where you might find it helpful to use the `sumArray` function you just wrote, then by all means....
...
A Prime number is an integer greater than 1 that is evenly divisible only by 1 and itself. If you want to read more deeply about it, [go here](https://en.wikipedia.org/wiki/Prime_number).
...
_Hint:_ Check every number up to the square root. To do this, try a `for` loop.
...
1. Instead of hardcoding the user input, let the user actually enter a choice with prompt().
...
<details><summary><strong>Digit Sum v2.0</strong></summary>

Write a function `sumDigits` that accepts a number and returns the sum of its digits, **without converting the number to a string first.**
...
2. Write a loop that console logs an "upside down left" isosceles triangle made of '#' that has the height and length of the argument.
...
3. Write a loop that console logs a "right isosceles" triangle (SEE BELOW) made of '#' that has the height and length of `argument`.  This is deceptively tricky. 
...
4. Write a loop that console logs an "upside down right" isosceles triangle made of '#' that has the height and length of the argument.  This is also tricky, but far less so once you get the previous triangle.
...
Answer the questions inside your `app.js` file in a folder structure like the one we've been using in class.

## Technical Requirements

1. The JavaScript file MUST run without syntax errors. 
2. Formatting must be perfect.  If you're not sure what we mean by perfect, go to [JS Beautifier](https://beautifier.io/) and paste in your code.  Press the button to fix the formatting and then **manually** edit your code to look like the output. 

## Submission Guidelines

- Submit your homework via a pull request.
- You are welcome to leave any questions or comments you have in the code. 
